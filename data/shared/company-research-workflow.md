...
- Prefer doing substantive local archive work only from the repo-declared canonical active workspace for that archive project.
...
When a project has a special local-workspace rule, the practical sequence is:

1. commit and push any important continuity work from the current checkout
2. refresh or recreate the repo inside the intended canonical workspace
