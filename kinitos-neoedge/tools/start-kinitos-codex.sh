...
ICLOUD_FRAGMENT="/Library/Mobile Documents/com~apple~CloudDocs/"
CANONICAL_WORKSPACE="/Users/stevenwoods/Library/Mobile Documents/com~apple~CloudDocs/StevenWoods/public-quack-recovery"
...
if [[ "$REPO_ROOT" != *"$ICLOUD_FRAGMENT"* ]] && [[ "$ALLOW_NON_ICLOUD" -ne 1 ]]; then
  fail "repo is not in an iCloud-backed path. Re-run from the canonical workspace or pass --allow-non-icloud for temporary verification only."
fi
...
KINITOS_STATUS="$(git -C "$REPO_ROOT" status --short -- kinitos-neoedge data/kinitos-neoedge data/projects/kinitos-neoedge.json || true)"
...
printf '\nNext intended phase:\n'
printf -- '- Portability validation first\n'
printf -- '- Preservation completeness second\n'
printf -- '- Broader research only after that\n'

if [[ "$REPO_ROOT" != *"$ICLOUD_FRAGMENT"* ]]; then
  warn "This run used --allow-non-icloud or a non-iCloud location. Treat it as a temporary validation, not the canonical active workspace."
fi
