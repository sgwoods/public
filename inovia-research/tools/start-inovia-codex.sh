...
need_file "$REPO_ROOT/inovia-research.html"
need_file "$SCRIPT_DIR/start-inovia-codex.sh"

BRANCH="$(git -C "$REPO_ROOT" rev-parse --abbrev-ref HEAD)"
COMMIT="$(git -C "$REPO_ROOT" rev-parse HEAD)"
REMOTE_URL="$(git -C "$REPO_ROOT" remote get-url origin)"
UPSTREAM_BRANCH="$(git -C "$REPO_ROOT" rev-parse --abbrev-ref --symbolic-full-name '@{u}' 2>/dev/null || true)"
INOVIA_STATUS="$(git -C "$REPO_ROOT" status --short -- inovia-research inovia-research.html || true)"
...
if [[ -n "$INOVIA_STATUS" ]]; then
  warn "Inovia subtree is not clean:"
  printf '%s\n' "$INOVIA_STATUS"
else
  printf 'Inovia subtree status: clean\n'
fi

printf '\nEra boundary rule:\n'
printf -- '- steven-woods-research owns the person-centric layer\n'
printf -- '- inovia-research owns the era-specific deep archive for the Inovia period\n'
printf -- '- overlap is allowed, but Inovia should keep its own deep interpretation and local preservation baseline\n'
...
printf '\nNext intended phase:\n'
printf -- '- Localize the current Inovia team profile\n'
printf -- '- Add the next public appearances and ecosystem-facing sources\n'
printf -- '- Keep Steven overlap explicit while growing the Inovia archive as its own era layer\n'
