...
(setq *initial*
	...
	(in-stat-formation White cp2 Coil 
      ...
      *i1* '(cp1
      ...
      ;for Mfls
      *g1* '(cp1
	     (started-on-leg White cp1 cp2 Move-From $tf $meth))
      ...
      ;for Column
      *i2-1* '(cp1
	       (started-on-leg White cp1 cp2 $movement Column $meth)
	       (NOT firing-on White $xy3 $direct-or-indirect
		    $xy1 $xy2 ))

      ;for Wedge-l
      *i2-2* '(cp1
	       (started-on-leg White cp1 cp2 $movement Wedge-l $meth)
	       (NOT firing-on White $xy3 $direct-or-indirect
		    $xy1 $xy2 ))

      ;for Wedge-l
      *g2* '(cp1
	     (in-trav-formation White cp1 cp2 Wedge-l $ff $fl $fr $meth))
      ...
      *i3-1* '(cp1
	       (started-on-leg White cp1 cp2 $movement $tf Trav-techn)
      ...
      *g3-1* '(cp1
      ...
      ;for Trav overwatch
      *i3-2* '(cp1
	       (started-on-leg White cp1 cp2 $movement $tf Trav-overw)
	       (in-trav-formation White cp1 cp2 $tf
      ...
      *g3-2* '(cp1
      ...
      ;for Nbd
      *i4* '(cp1
	     (moving-along-leg White cp1 cp2 $a $b Trav-techn)
      ...
      *g4* '(cp1
      ...
      ;for Mtle-nbo
      *i5-1* '(cp1
	       (moving-along-leg White cp1 cp2 $sp1 $sec1 Trav-techn)
      ...
      ;for Mtle-nbo
      *g5* '(cp1
	     (at-location White cp2))

      ;for Mtle-bo
      *i5-2* '(cp1
	       (moving-along-leg White cp1 cp2 $sp1 $sec1 Bound-overw)
	       (drill-scheduled)
	       (NOT firing-on White $xy3 $direct-or-indirect
		    cp1 cp2 ))
