;; Pautler CAETI 6 CHANGED Jan 31/97
...
(setq wltf
      (make-operator
       ...
       :name '(Wedge-left-traveling-formation ;;left flank is longer
	       $group1 $movement $xy1 $xy2 $method $group2 )
       :cost 1
       :stage 2
       :preconditions '(
...
(setq cfcr
      (make-operator
       ...
       :effects       '(
			;cribbed from FTG
			(firing-on $group $xy3 $direct-or-indirect $xy1 $xy2)
			;not used with cribs
...
(setq *critical-list* '(
   (2
    (= $ $) ;Predicates with attached procedures should appear at all levels
    (NOT = $ $) ; of the criticality list.
    (at-location     $ $)
    (NOT at-location $ $)
    (moving-along-leg $ $ $ $ $ $)
