;; Pautler CAETI 4 CHANGED Dec 10/96
...
(setq mfls
      (make-operator
       :opid 'mfls
       :name '(Move-from-leg-start $group $xy2 $xy3 $tform $method2
...
(setq mofsf
      (make-operator
       ...
       :name '(Move-out-from-stat-form $group $xy1 $xy2 $tform $method !
				       ...
				       $mv-out-speed )
       :cost 2 ;Pref should lean toward Mfls because most legs will be in the
       :stage 1  ; middle of a route/plan.
       :preconditions '(
...
(setq mofhtf
      (make-operator
       ...
       :name '(Move-out-from-halted-trav-formation
	       $group $xy2 $xy3 $tform2 $method2 !
	       $xy1 $tform1 $method1 $fire-f $fire-l $fire-r )
       :cost 3 ;DP thinks that this act is much less common than the other 2.
       :stage 1
       :preconditions '(
...
(setq ctf
      (make-operator
       :opid 'ctf
       :name '(Column-traveling-formation $group $xy1 $xy2 ! $movement $method)
       :cost 3
       :stage 2
       :preconditions '(
			(started-on-leg $group $xy1 $xy2
...
(setq vtf ;;; !!!!!!!!!!! The fire values for the Vee are only guesses
      (make-operator
       :opid 'vtf
       :name '(Vee-traveling-formation $group $xy1 $xy2 ! $movement $method)
       :cost 2
       :stage 2
       :preconditions '(
			(started-on-leg $group $xy1 $xy2
...
(setq mtle-nbo
      (make-operator
       :opid 'mtle-nbo
       :name '(Move-to-leg-end-nbo $group $xy2 ! $xy1 $speed $security
...
(setq mtle-bo
      (make-operator
       :opid 'mtle-bo
       :name '(Move-to-leg-end-bo $group $xy2 ! $xy1 $speed $security)
...
(setq htf
      (make-operator
       :opid 'htf
       :name '(Halt-trav-formation $group $xy2 ! $xy1 $speed $security
				   $tform $method $fire-f $fire-l $fire-r )
...
(setq sicoil
      (make-operator
       :opid 'sicoil
       :name '(Stop-in-coil $group $xy2 ! $xy1 $speed $security $method
...
(setq siherr
      (make-operator
       :opid 'siherr
       :name '(Stop-in-herringbone $group $xy2 ! $xy1 $speed $security $method
				   $xy3 $direct-or-indirect )
       :cost 2
       :stage 6
       :preconditions '(
...
(setq *precond-new-est-only-list*
	...
	;(leg-clear $a $b) ;leg-clear op used twice in some legs, so reuse
