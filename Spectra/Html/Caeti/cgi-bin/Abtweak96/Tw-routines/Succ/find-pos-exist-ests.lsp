; Tw-routines/Succ/find-pos-exist-ests.lsp
...
(defun find-pos-exist-ests (plan u p)
  "Tw-routines/Succ/find-pos-exist-ests.lsp   (oct 11)
  ...
  (let* ((all-poss-before (all-poss-before u plan))
	 (pos-est-pairs (find_pos_est_pairs plan all-poss-before u p)) )
    (declare (type list all-poss-before) (type (list list) pos-est-pairs))
...
(defun make-pos-intermediates (pos-est-pairs u p plan)
  "Tw-routines/Succ/find-pos-exist-ests.lsp     Code restructured Nov 22/96
   create list of (est p newplan),
   pos-est-pairs is a list of (est effect(est))."
  (declare (type (list list) pos-est-pairs) (type atom u) (type list p)
	   (type plan plan) )
  (and pos-est-pairs
       (cons
	(let* ((est     (get-establisher pos-est-pairs))
	       ;(ncp     (add-cost-to-plan 0 plan))  ; add in cost of generating en intermediate
	       (est-e   (get-establisher-effect pos-est-pairs)) 
	       (childpn (make-est plan est est-e u p)) ;'plan' was 'ncp'
	       )
		...
		(format *output-stream*
			"~%>>>~
			 ...
			 ~%>>> Considering effect:~
	       		 ...
	       		 ~%>>> of op: ~S~
                         ~%>>>"
			(plan-id plan) (plan-id childpn) 
			est-e
			(operator-name
			 (get-operator-from-opid est plan) )
			p
			(operator-name
...
(defun make-est (plan estid effect-of-est u p)
  ...
  (declare (type plan plan) (type atom estid) (type list effect-of-est)
	   (type atom u) (type list p) )

  (let ((new-plan (make-copy-of-plan plan)))
...
(defun find_pos_est_pairs (plan pos-ests u p)
  ...
  (declare (type plan plan) (type list pos-ests) (type atom u)
	   (type list p) )
  (and pos-ests
       (append
...
(defun find_est_effect_pairs_for_pos-est (plan pos-estid u p)
  ...
  (declare (type plan plan) (type atom pos-estid) (type atom u)
	   (type list p) )
  (unless (and (find p (get-effects-of-opid pos-estid plan) :test #'equal)
	       (nece-before-p pos-estid u plan) )
	  ;pos-estid is possible, but not necessarily yet.
	  (mapcar
	   #'(lambda (effect)
	       (when *plan-debug-mode*
		     (format *output-stream*
			     ...
			     effect
			     (operator-name
			      (get-operator-from-opid pos-estid plan) )))
...
(defun find-all-poss-codes-effects (pos-estid p plan)
 "Tw-routines/Succ/find-pos-exist-ests.lsp
    ...
    (declare (type atom pos-estid) (type list p) (type plan plan))
   (remove
    nil
    (mapcar #'(lambda (effect)
		(and (poss-codesignates-p p effect plan)
		     effect ))
