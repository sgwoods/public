;; Tw-routines/general.lsp

;; general use routines

(defun random-element1 (list)
   "Tw-routines/general.lsp
...
(defun instantiate (item1 mapping)
  "Tw-routines/general.lsp
   if item1 = ((p $x $y)...), mapping= (($x a) ($y b)) then return
   ((p a b)....)"			   
  (declare (list mapping))

  (cond ((var-p item1)
	 (or (second (assoc item1 mapping :test #'equalp))
	     item1 ))
	((atom item1) item1)
	((listp item1)
	 (mapcar #'(lambda (item1-1)
		     (instantiate item1-1 mapping) )
		 item1 ))
	(t item1) ))
...
(defun clean-up-mapping (substitution-list old-variables)
  "Tw-routines/general.lsp
   ...
   (declare (type list substitution-list) (type list old-variables))
