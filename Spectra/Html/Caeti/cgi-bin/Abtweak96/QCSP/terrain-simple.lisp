;; terrain-simple.lisp
;;
;;  CAETI Scenario Planner TerrainCSP Phase
;; 
;;               Steven Woods, David Pautler
;;               CAETI Project (Alex Quilici)
;;               December, 1996
;;
...
(setq *debug-dot* nil)
(defun cts () (compile-file "QCSP/terrain-simple")
  (load "QCSP/terrain-simple"))

;; ---------------------------------------------------------------------------
;; template components/slots, includes reflexive or node constraints

;; slot  pos slot-type    

; (ts1   xy1 EnemyPosition HelicopterBase )
; (ts2   xy2 EnemyPosition HelicopterAttack )

; (ts3   xy3 MovementLegStart )
; (ts4   xy4 MovementLegEnd   )

;; ---------------------------------------------------------------------------
;; template constraints, ie arc or inter-node constraints 

;with constraints like ...
; (distance-from-point     ts1 ts2 10)       ; 10 is the max base to attack range say

; (exists-obstacle-between ts1 ts2 (Mountain) )  ; cover for base exists

; (vector-intersection     ts1 ts2 ts3 ts4 )     ; 4-ary constraint
    ;
    ; vector extending from ts1 through ts2 crosses a segment defined by
    ;  endpoints ts3 and ts4; there is not necessarily any movement involved

; (distance-from-line      ts2 ts3 ts4 Distance) ; 3-ary  constraint
    ;
    ; ts2 lies within Distance of a segment defined by endpoints ts3 and
    ;  ts4; there is not necessarily any movement involved

;; ---------------------------------------------------------------------------
;; START DOMAIN FUNCTIONS HERE .....
;; ---------------------------------------------------------------------------

(defun tcsp (&key
	     (situation-id    "terrain-1") 
	     (sit-noise                 0)
	     (template-id   "terrain-templ1")  
	     (dom (list 'tcsp situation-id template-id))
	     (search-mode            "bt")  ;; backTRACK/JUMP/MARK bt bj bm
	     (node-consis               t)  ;; make node consis before search
	     (node-type-consis        nil)
	     ...
	     (output-file     "no-output")  ;; write output to
	     (long-output             nil)  ;; force LONG output explanation
	     (single-line-override    nil)  ;; overrides other out, 1 line only
	     (load-real-terrain-db    nil)  ;; load real terrain database (large)
	     )
  "TCSP version v1.0.  Takes no required args, but
   it is assumed we will use the default situation and template values."

  (setq *check* 1)
  (format *standard-output* "check point 1~&")

  ;; insure TERRAIN functions are loaded
  ;; arc-p consistent-p 
  ;;(unless (eq *domain-loaded* 'terrain)
  ;;        (load "QCSP/terrain-simple")
  ;;        (load "QCSP/terrain-setup")       ; re-load terrain-setup constants
  ;;        (load "QCSP/terrain-setup-dp")    ; defines *situations*
  ;;        )

  (when (and load-real-terrain-db
	     (not *terrain-realdb-loaded*) )
	(format *standard-output*
		"Delay 1 - loading large CAETI spatial database.~&" )
	(load-mapdb)  ; loads map array into global *mapdb
	(format *standard-output*
		"Delay 2 - loading large CAETI spatial index.~&" )
	(load-mapndx) ; loads map index by type into *mapndx* & type globals
	(setq *terrain-realdb-loaded* t)
	(format *standard-output* "Done loading db.~&")
	(push '("real-terrain-db" (dummy  (0 0)  Plain 5+)) 
	      *situations* ))

  (setq *test* situation-id)
  (setq *check* 2)
  (format *standard-output* "check point 2~&")
  ...
  ;; Establish initial terrain situations
  ...
  (if (tcsp-set-global-values situation-id 
			      sit-noise 
			      random-ident 
			      template-id 
			      sch-call 
	...
	;; The terrain generation now occurs in the caeti demo
	;; (save-gnuplot   random-ident sit-noise)
	)
    (progn
      (comment "Exiting with a TERRAIN setup error.")
      (return-from tcsp nil) ))

  (setq *check* 3)
  (format *standard-output* "check point 3~&")

  ;; set up global variables
  (unless (set-globals (tcsp-variables)
		       dom 
		       search-mode 
		       node-consis 
		       node-type-consis
  ...
  (setq *check* 4)
  (format *standard-output* "check point 4~&")

  ;; Show set up options
  (show-options)

  (let ((init-var-set *raw-variables*)         ;init node consis via tcsp-var
	(arc-call     (or (eq arc-consis 'during)
			  (eq arc-consis 'both) ))
	(arc-pre      (or (eq arc-consis 'before)
			  (eq arc-consis 'both) )))

    (format *standard-output* "check point 5~&")

    (var-set-characterize init-var-set 'initial)

    (format *standard-output* "check point 5b~&")

    ;; Time advance constraint processing time
    (setq *internal-advance-start-time* (get-internal-run-time))

    (cond ((and node-consis arc-pre)
	   (format *standard-output* "check point 5-bad1~&")
	   (setq init-var-set 
		 (ac-3
		  (node-consistent-variables
		   init-var-set 
	  ...
	  (node-consis
	   (format *standard-output* "check point 5c~&")
	   (setq init-var-set
		 (node-consistent-variables init-var-set 
					    :force node-force-all)) 
	   (var-set-characterize init-var-set 'node ) )
   
	  (arc-pre
	   (format *standard-output* "check point 5-bad2~&")
	   (setq init-var-set
		 (ac-3 init-var-set #'arc-p #'consistent-p nil) )
	   (var-set-characterize init-var-set 'ac3)
	   (setq *arc-constraint-fail* *constraint-fail*)
	   (setq *constraint-fail* nil) )
	  ) ;; cond

    (format *standard-output* "check point 6~&")
	...
	(if (eq advance-sort 'random)
	    ;; random ordering
	    (progn
	      (format *standard-output* "check point 6b-random-vsrt~&")	    
	      (setq init-var-set (random-order init-var-set)) )
	  ;; advance-sort ordering by heuristic 
	  (progn
	    (format *standard-output* "check point 6b-heur-vsrt~&")	    
	    (setq *internal-sort-start-time* (get-internal-run-time)
		  init-var-set (advance-sort init-var-set)
		  *internal-sort-end-time* (get-internal-run-time) ))))

    (format *standard-output* "check point 7~&")
    ...
    (setq *variables* init-var-set
	  *var-order* (mapcar #'(lambda (x) (first x)) init-var-set)
	  *var-order-note* 'redone )

    (format *standard-output* "check point 8 - bt start~&")
...
(defun tcsp-variables ( &optional (template *current-template*) )
  "For each slot in the template, find IDs for all terrain coords (sitobjs)
   that match the terrain-type.  The result is an a-list keyed by template
   slot IDs (coord tokens)."
  (let ((plain-values    (get-sit-ids :type 'plain))
	(forest-values   (get-sit-ids :type 'forest))
	(mountain-values (get-sit-ids :type 'mountain)) )
    (mapcar                     ;v some subset of (Plain Forest Mountain)
     #'(lambda (templ-slot &aux (slot-types (second templ-slot)))
	 (cons (get-templ-id templ-slot)
	       (append
		(and (find 'plain slot-types) plain-values)
		(and (find 'forest slot-types) forest-values)
		(and (find 'mountain slot-types) mountain-values) )))
     (get-templ-slots template) ))) ;node constraints

(defun tcsp-variables-old ( &optional (template *current-template*) )
  (let ((allvalues (get-sit-ids))
	(varlist nil) )
...
(defun node-consistent-variables (variable-list &key (force nil) )
  "Accept domain variable list, return the list in node consistent form only.
   This dramatically reduces search in a single pass."
  ...
  (mapcar
   #'(lambda (node)
       (node-consistent-one-node node :force force) )
   variable-list ))
...
(defun node-consistent-one-node ( node &key (force nil) )
  ...
  (when *debug-dot* (format *standard-output* "~&Var = ~A~&" (first node)))
  (let ((variable (car node))
	(domain   (cdr node)) )
    (list variable
	  (apply
	   #'append
...
(defun node-consistent-p (tslot-id sit-obj-id &key (force nil) )
  "Shortened version - spatial type info computed in index phase. 
   Note this version also does not record failures."

  (if *terrain-realdb-loaded*
      (let ((sit-obj (get-sit-object        sit-obj-id))
	    (tslot   (get-templ-slot-object *current-template* tslot-id)) )
	(incf *node-consistency-calls*)
	(if (every #'(lambda (test-fn)
		       (funcall test-fn tslot sit-obj) )
		   *node-consistency-test-fns* )
	    (prog1 t
	      (incf *node-consistency-cks-ok*) )
	  nil ))
    (node-consistent-p-old tslot-id sit-obj-id force) ))

;; ***************************************************************************

(defun node-consistent-p-old (tslot-id sit-obj-id &key (force nil) )
  "Does this sit-obj id pass the constraints for tslot ?  This is used in
   initial domain assigment to each template slot.
   Note "force" forces the computation of ALL node arcs, default is only
   until a fail is found.
   This is the TCSP version."
   ...
   (let ((sit-obj (get-sit-object     sit-obj-id))
	 ...
	 (let ((match-type     (tcsp-matches-type       tslot sit-obj))
	       (match-radial   (tcsp-matches-radial     tslot sit-obj))
	       (match-abs-loc  (tcsp-matches-abs-loc    tslot sit-obj)) )
	   ...
	   (record-node-fail tslot-id sit-obj-id 
			     ...
			     match-radial
			     match-abs-loc 
			     t                    ; place holder atom type 3
			     t                    ; place holder atom type 4
			     )

	   (and match-type match-radial match-abs-loc) )
       ...
       (if (tcsp-matches-type        tslot sit-obj)
	   (if (tcsp-matches-radial      tslot sit-obj)
	       (if (tcsp-matches-abs-loc       tslot sit-obj)
		   (if t       ; place holder atom type 3
		       (if t       ; place holder atom type 4

			   ;; ONLY SUCCESS CASE - all atom type match instance
			   (progn
...
(defun arc-p (symbol1 symbol2)
  ...
  (some
   #'(lambda (start-end-coord-pair) ;v does not discriminate for arc-direction
       (find-both-p symbol1 symbol2 start-end-coord-pair) )
   (mapcar #'second ;yields <start-coord end-coord> pairs
...
(defun consistent-p (ts1 sit1 ts2 sit2 partial-solution 
  ...
  (if (equal sit1 sit2)
      (progn
	(incf *unique-restrict-count*)
	(when *debug-dot* (output-80 "u"))
	(when *debug-consis* (comment3 "Restricted unique for" sit1 sit2 nil))
	(let (
...
(defun sort-constraint-list (constraint-list)
  "A sort will affect the perform of the TCSP.  We would wish to put
   constraints more likely to fail FIRST in the list, thus way avoiding the
   checking of arc constraints that usually succeed.  We must balance this
   with the list sorting cost, and also keeping in mind that the *best* order
   for a particular pair of template slots may be difficult (?) to
   determine... more work required here."
...
(defun consistent-all (ts1 s1 ts2 s2 blist cs)
  "Check entire set of constraints for this pair of bindings.
   If ANY constraint fails, return (FALSE failure-level), else 
   return (TRUE 0)."

  (if (null cs)
      (list 't 0)
    (let* ((result-all (test-constraint-1 ts1 s1 ts2 s2 blist (car cs))  )
	...
	(progn
	  (if *debug-dot* (output-80 "x"))
	  (list nil level)) ))))

(let ( (line 1) )
  (defun output-80 ( char1 &key (reset nil) )
    (if reset
	(progn
	  (format *standard-output* "~&~A" char1)
	  (setq line 2) )
      (if (= line 130)
	  (progn
	    (format *standard-output* "~A~&" char1)
	    (setq line 1))
	(progn
	  (format *standard-output* char1)
	  (setq line (1+ line) ))))))

;; **************************************************
;;
(defun test-constraint-1 (ts1 s1 ts2 s2 blist constraint)
  "Does this single constraint hold for the assignment of s1 to ts1 and s2
   to ts2 ?
  ...
  (let ((cons-level (get-constraint-max-level constraint ts1))
...
(defun test-constraint-2 (ts1 s1 ts2 s2 blist constraint)
  (let ((cons-type  (get-constraint-type      constraint)))
    ...
    (cond   ;; Constraint type
     ...
     ;; ----------------------------------------------------------------------
     ;; TCSP Constraint Additions Start
     ;; ----------------------------------------------------------------------

     ((eq cons-type 'leg-clear)
      (let* ((spx (get-sit-obj-x s1))
	     (spy (get-sit-obj-y s1))
	     (epx (get-sit-obj-x s2))
	     (epy (get-sit-obj-y s2))
	     (gs-list (grid-squares-between spx spy epx epy)) )
	(setq *constraint-cks* (+ 1 *constraint-cks*))
	(when *debug-consis* (comment1 "CC Leg Clear, cnt= " *constraint-cks*))
	(setq *constraint-legclr-cks* (+ 1 *constraint-legclr-cks*))

	;; Make sure no Mountain occupies any grid square on the route
	(if (terrain-in-gs-list 'Mountain gs-list)
	    (progn
	      (when  *debug-consis* (comment2 "Leg Clear fail for" s1 s2))
	      nil )
	  (progn
	    (when *debug-consis* (comment2 "Leg Clear succeed for" s1 s2))
	    t ))))

     ((eq cons-type 'leg-lateral-space)
      (let* ((min-lat-spc (get-nth-param 1 (get-parameter-list constraint)))
	     (spx (get-sit-obj-x s1))
	     (spy (get-sit-obj-y s1))
	     (epx (get-sit-obj-x s2))
	     (epy (get-sit-obj-y s2))
	     (gs-list (grid-squares-between spx spy epx epy)) )
	(setq *constraint-cks* (+ 1 *constraint-cks*))
	(when *debug-consis* (comment1 "CC LLS, cnt= " *constraint-cks*))
	(setq *constraint-leglatspc-cks* (+ 1 *constraint-leglatspc-cks*))

	;; Make sure all grid squares on the route have adequate lls
	(if (all-gs-sat-lat-space min-lat-spc gs-list)
	    (progn
	      (when *debug-consis*
		    (comment2 "Leg Lateral Space succeed for" s1 s2) )
	      t )
	  (progn
	    (when *debug-consis* (comment2 "Leg Lateral Space fail for" s1 s2))
	    nil ))))

     ((eq cons-type 'xloc-greater-than)
      (let* ((spx (get-sit-obj-x s1))
	     (epx (get-sit-obj-x s2)) )
	(setq *constraint-cks* (+ 1 *constraint-cks*))
	(when *debug-consis* (comment1 "CC XGT, cnt= " *constraint-cks*))
	(setq *constraint-xgt-cks* (+ 1 *constraint-xgt-cks*))

	(if (>= spx epx)
	    (progn
	      (when *debug-consis* (comment2 "XGT succeed for" s1 s2))
	      t )
	  (progn
	    (when *debug-consis* (comment2 "XGT fail for" s1 s2))
	    nil ))))

     ((eq cons-type 'yloc-greater-than)
      (let* ((spy (get-sit-obj-y s1))
	     (epy (get-sit-obj-y s2)) )
	(setq *constraint-cks* (+ 1 *constraint-cks*))
	(when *debug-consis* (comment1 "CC XGT, cnt= " *constraint-cks*))
	(setq *constraint-xgt-cks* (+ 1 *constraint-xgt-cks*))
	...
	(if (>= spy epy)
	    (progn
	      (when *debug-consis* (comment2 "YGT succeed for" s1 s2))
	      t )
	  (progn
	    (when *debug-consis* (comment2 "YGT fail for" s1 s2))
	    nil ))))
     ...
     ((eq cons-type 'sep)
      (let ((dist (distance-between s1 s2))
	    (minimum (get-nth-param 1 (get-parameter-list constraint) ))
	    (maximum (get-nth-param 2 (get-parameter-list constraint) )))
	(setq *constraint-cks* (+ 1 *constraint-cks*))
	(when *debug-consis* (comment1 " " *constraint-cks*))
	(setq *constraint-sep-cks* (+ 1 *constraint-sep-cks*))
      
	(if (and (>= dist minimum)    ;; greater than minimum separation
		 (<= dist maximum))   ;; less than maximum sepration
	    (progn
	      (when *debug-consis* (comment2 "Separation succeed for" s1 s2))
	      t)
	  (progn
	    (when *debug-consis* (comment2 "Separation fail for" s1 s2))
     ...
     ((eq cons-type 'same-attr-type)
      ...
      (let ((s1-type (get-sit-obj-type s1))
	    (s2-type (get-sit-obj-type s2)) )
	(if (equal s1-type s2-type)
	    (progn
	      (when *debug-consis*
		    (comment2 "Same attr type succeed for" s1 s2) )
	      t)
	  (progn
	    (when *debug-consis* (comment2 "Same attr type fail for" s1 s2))
	    (record-fail ts1 s1 ts2 s2 constraint)
	    nil ))))
     ...
     ((eq cons-type 'same-attr-orient)
      ...
      (let ((s1-orient (get-sit-obj-orient s1))
	    (s2-orient (get-sit-obj-orient s2))	)
	(if (equal s1-orient s2-orient)
	    (progn
	      (when *debug-consis*
		    (comment2 "Same attr orient succeed for" s1 s2) )
	      t)
	  (progn
	    (when *debug-consis* (comment2 "Same attr orient fail for" s1 s2))
	    (record-fail ts1 s1 ts2 s2 constraint)
	    nil ))))
     ...
     ((eq cons-type 'same-attr-activity)
      (setq *constraint-cks* (+ 1 *constraint-cks*))
      (when *debug-consis* (comment1 " " *constraint-cks*))
      (setq *constraint-same-activity-cks*
	    (+ 1 *constraint-same-activity-cks*) )
      (let ((s1-activity (get-sit-obj-activity s1))
	    (s2-activity (get-sit-obj-activity s2)) )
	(if (equal s1-activity s2-activity)
	    (progn
	      (when *debug-consis*
		    (comment2 "Same attr activity succeed for" s1 s2) )
	      t)
	  (progn
	    (when *debug-consis*
		  (comment2 "Same attr activity fail for" s1 s2) )
	    (record-fail ts1 s1 ts2 s2 constraint)
	    nil ))))
     ...
     ((eq cons-type 'same-attr-size)
      ...
      (let ((s1-size (get-sit-obj-size s1))
	    (s2-size (get-sit-obj-size s2)) )
	(if (equal s1-size s2-size)
	    (progn
	      (when *debug-consis*
		    (comment2 "Same attr size succeed for" s1 s2) )
	      t)
	  (progn
	    (when *debug-consis* (comment2 "Same attr size fail for" s1 s2))
	    (record-fail ts1 s1 ts2 s2 constraint)
	    nil ))))
     ...
     ((eq cons-type 'echelon)
      ...
      (let* ((affectlist (get-affected-list constraint))
	     (p1        (get-nth-param 1 affectlist))    ;; first  tslot id
	     (p2        (get-nth-param 2 affectlist))    ;; second tslot id
	     (bound     (get-nth-param 1 (get-parameter-list constraint))) )
	(cond ((and (eq ts1 p1)
		    (eq ts2 p2) )
	       (if (echelon-bound-p (get-sit-obj-x s1)
				    (get-sit-obj-y s1)
		   ...
		   (progn
		     (when *debug-consis*
			   (comment3 "Echelon succeed for" s1 s2 bound) )
		     t)
		 (progn
		   (when *debug-consis*
			 (comment3 "Echelon Fail for" s1 s2 bound) )
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil) ))

	      ((and (eq ts1 p2) (eq ts2 p1))
	       (if (echelon-bound-p (get-sit-obj-x s2)
				    (get-sit-obj-y s2)
		 ...
		 (progn
		   (when *debug-consis*
			 (comment3 "Echelon Fail for" s2 s1 bound) )
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      (t
	...
	(cond
	 ...
	 ((and (equal ts1 p1) (equal ts2 p2))
	  (let ((sp3 (get-tslot-instant p3 blist)))
	    (if (eq sp3 nil)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(1) NIL succeed for" s1 s2 sp3) )
		  t)
	      (if (medial-distance-bound-p (get-sit-obj-x s1)
					   (get-sit-obj-y s1)
					   (get-sit-obj-x s2)
					   (get-sit-obj-y s2)
					   (get-sit-obj-x sp3)
		  ...
		  (progn
		    (when *debug-consis*
			  (comment3 "Medial(1) succeed for" s1 s2 sp3) )
		    t)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(1) fail for" s1 s2 sp3) )
		  (record-fail ts1 s1 ts2 s2 constraint)
		  nil )))))
	 ...
	 ((and (equal ts1 p2) (equal ts2 p1))
	  (let ((sp3 (get-tslot-instant p3 blist)))
		...
		(progn
		  (when *debug-consis*
			(comment3 "Medial(2) NIL succeed for" s2 s1 sp3) )
		  t)
	      (if (medial-distance-bound-p (get-sit-obj-x s2)
					   (get-sit-obj-y s2)
					   (get-sit-obj-x s1)
					   (get-sit-obj-y s1)
					   (get-sit-obj-x sp3)
		  ...
		  (progn
		    (when *debug-consis*
			  (comment3 "Medial(2) succeed for" s2 s1 sp3) )
		    t)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(2) fail for" s2 s1 sp3) )
		  (record-fail ts1 s1 ts2 s2 constraint)
		  nil )))))

	 ;; Case 3/6
	 ((and (equal ts1 p1) (equal ts2 p3))
	  (let ((sp2 (get-tslot-instant p2 blist)))
	    (if (eq sp2 nil)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(3) NIL succeed for" s1 sp2 s2) )
		  t)
	      (if (medial-distance-bound-p (get-sit-obj-x s1)
					   (get-sit-obj-y s1)
					   (get-sit-obj-x sp2) 
		  ...
		  (progn
		    (when *debug-consis*
			  (comment3 "Medial(3) succeed for" s1 sp2 s2) )
		    t)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(3) fail for" s1 sp2 s2) )
		  (record-fail ts1 s1 ts2 s2 constraint)
		  nil )))))
	 ...
	 ((and (equal ts1 p3) (equal ts2 p1))
	  (let ((sp2 (get-tslot-instant p2 blist)))
		...
		(progn
		  (when *debug-consis*
			(comment3 "Medial(4) NIL succeed for" s2 sp2 s1) )
		  t)
	      (if (medial-distance-bound-p (get-sit-obj-x s2)
					   (get-sit-obj-y s2)
					   (get-sit-obj-x sp2) 
		  ...
		  (progn
		    (when *debug-consis*
			  (comment3 "Medial(4) succeed for" s2 sp2 s1) )
		    t)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(4) fail for" s2 sp2 s1) )
		  (record-fail ts1 s1 ts2 s2 constraint)
		  nil )))))

	 ;; Case 5/6
	 ((and (equal ts1 p3) (equal ts2 p2))
	  (let ((sp1 (get-tslot-instant p1 blist)))
	    (if (eq sp1 nil)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(5) NIL succeed for" sp1 s2 s1) )
		  t)
		  ...
		  (progn
		    (when *debug-consis*
			  (comment3 "Medial(5) succeed for" sp1 s2 s1) )
		    t)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(5) fail for" sp1 s2 s1) )
		  (record-fail ts1 s1 ts2 s2 constraint)
		  nil )))))
	 ...
	 ((and (equal ts1 p2) (equal ts2 p3))
	  (let ((sp1 (get-tslot-instant p1 blist)))
		...
		(progn
		  (when *debug-consis*
			(comment3 "Medial(6) NIL succeed for" sp1 s1 s2) )
		  t)
	      (if (medial-distance-bound-p (get-sit-obj-x sp1) 
					   (get-sit-obj-y sp1)
					   (get-sit-obj-x s1)
		  ...
		  (progn
		    (when *debug-consis*
			  (comment3 "Medial(6) succeed for" sp1 s1 s2) )
		    t)
		(progn
		  (when *debug-consis*
			(comment3 "Medial(6) fail for" sp1 s1 s2) )
		  (record-fail ts1 s1 ts2 s2 constraint)
		  nil )))))
	...
	(cond ((and (eq ts1 p1) (eq ts2 p2))
	       (if (sit-object-right-of-p s1 s2)
		   (progn
		     (when *debug-consis*
			   (comment2 "Right-of succeed for" s1 s2) )
		     t)
		 (progn
		   (when *debug-consis* (comment2 "Right-of failed for" s1 s2))
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      ...
	      ((and (eq ts1 p2) (eq ts2 p1))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Right-of succeed for" s2 s1) )
		     t)
		 (progn
		   (when *debug-consis* (comment2 "Right-of failed for" s2 s1))
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      (t
	...
	(cond ((and (eq ts1 p1) (eq ts2 p2))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Left-of succeed for" s1 s2) )
		     t)
		 (progn
		   (when *debug-consis* (comment2 "Left-of failed for" s1 s2))
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      ...
	      ((and (eq ts1 p2) (eq ts2 p1))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Left-of succeed for" s2 s1) )
		     t)
		 (progn
		   (when *debug-consis* (comment2 "Left-of failed for" s2 s1))
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      (t
	...
	(cond ((and (eq ts1 p1) (eq ts2 p2))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Ahead-of succeed for" s1 s2) )
		     t)
		 (progn
		   (when *debug-consis* (comment2 "Ahead-of failed for" s1 s2))
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      ...
	      ((and (eq ts1 p2) (eq ts2 p1))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Ahead-of succeed for" s2 s1) )
		     t)
		 (progn
		   (when *debug-consis* (comment2 "Ahead-of failed for" s2 s1))
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      (t
	...
	(cond ((and (eq ts1 p1) (eq ts2 p2))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Behind-of succeed for" s1 s2) )
		     t)
		 (progn
		   (when *debug-consis*
			 (comment2 "Behind-of failed for" s1 s2) )
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      ...
	      ((and (eq ts1 p2) (eq ts2 p1))
		   ...
		   (progn
		     (when *debug-consis*
			   (comment2 "Behind-of succeed for" s2 s1) )
		     t)
		 (progn
		   (when *debug-consis*
			 (comment2 "Behind-of failed for" s2 s1) )
		   (record-fail ts1 s1 ts2 s2 constraint)
		   nil )))
	      (t
...
;; ***************************************************************************
;; New Routines for CAETI domain constraint functions    Dec/96 sgw/dp
;; ***************************************************************************

(defun terrain-in-gs-list (terrain-type gs-list)
  "Returns true iff terrain-type is the type of at least one of the grid
   squares in gs-list."
  (dolist (gs gs-list nil)
	  (when (equal (get-terrain-from-coord gs) terrain-type)
		(return t))))

(defun get-terrain-from-coord (gs)
  (get-sit-type (get-sitobj-from-coord gs) ))

(defun all-gs-sat-lat-space  (min-lat-spc gs-list)
  "Returns true iff all grid squares satisfy the minimum lateral space
   requirement given in min-lat-spc."
   (dolist (gs gs-list t)
	   (when (not (lat-space-gte (get-radial-from-coord gs) min-lat-spc))
		 (return nil)) ))

(defun get-radial-from-coord (gs)
  (get-sit-radial (get-sitobj-from-coord gs) ))

;; assume 5+ is greater than anything except 5+, also nonnumberp => 5+
(defun lat-space-gte (ls1 ls2)
  (if (and (numberp ls1) (numberp ls2))
      (> ls1 ls2)
    (if (and (eq ls1 '5+) (eq ls2 '5+))
	t  ; equal 
      (if (and (eq ls1 '5+) (numberp ls2))
	  t ; ls1 greater
	(if (and (numberp ls1) (eq ls2 '5+))
	    nil ; ls2 greater
	  (progn
	    (comment2 "Error in lat-space-gte..." ls1 ls2)
	    nil
	    ))))))

;;
;; Note: hash table fns, ie (get-sitobj-from-coord coord) defined in terrain-setup
;;

(defun grid-squares-between (x1 y1 x2 y2
                                  &aux (x-diff (- x2 x1))
                                  (gs-list nil) )
  "Returns a list of grid squares between start-point and end-point."
  (cond ((= x-diff 0) ;avoid division by zero (i.e. the slope is infinite)
         (let (y3 y4)
           (if (> y2 y1)
               (setq y3 y1
                     y4 y2 )
        ...
        (t
         (let* ((slope (/ (- y2 y1) x-diff))
                x-start y-start x-end y-end y-diff flat-or-ascending? )
           (if (> x-diff 0)
               (setq x-start x1
                     y-start y1
                     x-end x2
...
(defun tcsp-matches-type (tslot sit-obj)
  "Does template slot match situation object for type value ?"

  (setq *node-consistency-checks* (+ 1 *node-consistency-checks*)
	*ts-match-type-count* (+ 1 *ts-match-type-count*) )
  (let ((ts-type        (get-tslot-type tslot))
	...
	(find so-type ts-type :test #'equalp)
	  ...
	  t
	(equal ts-type so-type) ))))

(defun tcsp-matches-radial (tslot sit-obj)
  "Does template slot match situation object for activity value ?"
  (setq *node-consistency-checks* (+ 1 *node-consistency-checks*)
	*ts-match-radial-count* (+ 1 *ts-match-radial-count*) )
  (let ((ts-radial     (get-tslot-radial tslot))
	(so-radial     (get-sit-radial   sit-obj)) )
    (if (listp ts-radial)   ;; is ts-activity a list of options ?
	(find so-radial ts-radial :test #'equalp)
      (if (eq ts-radial '*)  ; wildcard
	  t
	(equal ts-radial so-radial) ))))

(defun tcsp-matches-abs-loc (tslot sit-obj)
  "Does template slot match situation object for abs loc range ?"
  (incf *node-consistency-checks*)
  (incf *ts-match-abs-loc-count*)
  (let ((loclist (get-tslot-abs-loc tslot)))
    (if (eq loclist '*)  ;; wildcard
	t
      (let ((min-x  (first  loclist))
	...
	(not (null (in-range-p so-x so-y min-x min-y max-x max-y))) ))))

;; ***************************************************************************
;; ***************************************************************************
;; Object accessors for TCSP domain
;; ***************************************************************************
;; ***************************************************************************
...
;; Slot List
;;
(defun get-templ-slots (template) ;node constraints
  (second template) )
...
;; Constraints
;;
(defun get-templ-constraints (template) ;arc constraints
  (third template) )
...
(defun get-templ-object ( ident )
  ...
  (and *template-object-list*
       (find ident *template-object-list*
	     :key #'get-templ-id :test #'equal )))
...
(defun split-key ( str-key )
  (let ((x (parse-integer str-key :start 0 :end 3))
	(y (parse-integer str-key :start 4)) )
    (list x y) ))

(defun form-sit-object ( this-key x y )
  (let* ((arr-entry (get-map-xy *mapdb* x y))
	 (type      (resolve-pt (third arr-entry)))
	 (wide      (fourth arr-entry))
	 (rays      (fifth arr-entry)) )
    (list this-key (list x y) type wide rays) ))

(defun get-sit-object ( ident )
  "Return sit object based on identifier."
  (if *terrain-realdb-loaded*
      (let ((this-key (split-key ident)))
	(form-sit-object ident (first this-key) (second this-key)))
    (find-sit-object *current-situation* ident)))

(defun get-sit-ids (&key (type nil) )
  "Return a list of all situation objects in global storage."
  (if (and type *terrain-realdb-loaded*)
      (case type
	    (Plain    *real-terrain-plain-ids*)
	    (Forest   *real-terrain-forest-ids*)
	    (Mountain *real-terrain-mountain-ids*) )
    (get-ids *current-situation*) ))

(defun get-ids (list)
  "Return a list of ids of a list of situation objects."
  (and (consp list)
       (cons (get-sit-id (car list))
	     (get-ids (cdr list)) )))
...
(defun get-sit-obj-radial (ident)
  "Given sit id, return radial-space."
  (get-sit-radial (get-sit-object ident)))
...
;; Radial Space Annotation in the Spatial Database
;;
(defun get-sit-radial (sit-obj)
  "Return radial space of sit-obj. IE how much space around it"
  (fourth sit-obj) )
...
;; ***************************************************************************
;; Spatial routines for TCSP domain
;;   Functions for the computation of spatial relationships for TCSP
;; ***************************************************************************
...
(defun echelon-bound-p ( x1 y1 orient x2 y2 bound )
  "Given a line defined as corresponding to the vector of orientation of
   the object at (x1 y1), is the object at (x2 y2) within bound of the line ?"
  ...
  (let* ((newxy (find-other-point x1 y1 orient))
...
(defun find-other-point (x1 y1 orient)
  ...
  (let* ((deltaxy (change-orient orient))
...
(defun in-range-p (px py min-x min-y max-x max-y)
  ...
  (let* ((x1 min-x)  ;; bottom left
    ...
    (point-poly-intersect-p x1 y1 x2 y2 x3 y3 x4 y4 px py) ))
...
(defun point-poly-intersect-p ( x1 y1 x2 y2 x3 y3 x4 y4 px py )
  ...
  (let ((d12 (vec-pt-interior-p x1 y1 x2 y2 px py))
...
(defun exceed-bound (this min-x max-x min-y max-y)
  (setq *constraint-cks* (+ 1 *constraint-cks*)
	*constraint-sch-cks* (+ 1 *constraint-sch-cks*) )
  (let ((this-x (get-sit-obj-x this))
