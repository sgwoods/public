...
(defun loadit (&optional (sys *unix*) ) 
  (if sys                ;; unix load
    (progn
      ...
      ;; load MPR domain
      ;;(load "QCSP/mpr-simple")   ;; functionality
      ;;(load "QCSP/mpr-setup")    ;; template, situation information

      ;; load ADT domain
      (load "QCSP/adt-simple")   ;; functionality
      (load "QCSP/adt-setup")    ;; template, situation information

      ;; load TERRAIN/CAETI domain
      (load "QCSP/terrain-simple")   ;; functionality
      (load "QCSP/terrain-setup")    ;; template, situation information
      (load "QCSP/terrain-setup-dp")    ;; template, situation information
      (load "QCSP/caeti-db-all")     ;; database array for map
      (load-mapdb)
      (load-mapndx)

      ;; load quilici search single phase
      (load "QCSP/quilici-search") 

      ;; load new memory-csp in two phases
      (load "QCSP/memory-csp")
      )
