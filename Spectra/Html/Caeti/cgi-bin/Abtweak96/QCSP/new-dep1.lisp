	...
	(let* ((parents-list (get-parents-list comp-index)))
	  (cond ((endp parents-list) nil) ;; *all-var-of-this-func* ) 
		((eq (length parents-list) 1)   
		 (i-modify-dependency (first parents-list)))
		((eq (length parents-list) 2)
		 (let ((first-dep (i-modify-dependency (first parents-list)))
		       (second-dep (i-modify-dependency (second parents-list))))
...
(defun modify-open-list (this-comp open-list )
  (let* ((children (nth 0 (get-comp-hash this-comp))) 
    ...
    (cond ((NULL children) temp-open-list)
	  ((atom children) 
		(add-atom-to-list-if-not-in children temp-open-list)) 
...
;;***********************************************************************************
(defun modify-open-list-for-non-loop (this-comp open-list )
