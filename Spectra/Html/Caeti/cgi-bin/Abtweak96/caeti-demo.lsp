...
(defun caeti-demo (&optional (quiet? t) (i *initial*) (g *goal*))
  (format t "~%~
...
(defun wait-for-user (prompt-string)
  (clear-input)
  (format t "~%~
             ~% ~a (hit RETURN)" prompt-string )
  (read-char)
  (values) )

(defun planner-demo (i g &key quiet? succ-continuation fail-continuation)
  (format t "~%~
  ...
  (let ((i-terrain-filters ;the csp needs constrs on the initial spot also
	 (terrain-filter-props    ;a] assume only 1 op, not a plan, is adequate
	  (operator-preconditions ;b] assume 1st op found is adequate
	   (caar (select-ops *operators* (car initial-state))) )))
    ...
    ;add a 'pt-separation' constraint or a route-lengthening heuristic, too
    (print 'add-separation-constraint)
    ;make the following list a file called "terrain-templ-dp" or whatever
    (list node-constraints arc-constraints nil)
    ) ;let
  ) ;defun
      ...
      (cond ((funcall test
	    ...
	    ((funcall test
	     ...
	     (setf (car pairs) (append (car tail-index)
		   ...
		   success? t ))
	    (t
	     (setf skipped-any-before-succ? (or skipped-any-before-succ? (not succ?))
		   succ? nil )))
...
|#
(defun my-copy-alist (a-list) ;'copy-alist' reuses the cdr of each pair, making it useless
  (mapcar #'copy-list a-list) )

(defun make-ascending-numbers-list (len &aux (count 0))
  (mapcar #'(lambda (foo) (incf count)) (make-list len)) )
