...
(((_is-able-to-do_ ?Person ?A) AND ;don't consider acts beyond your competence
...
(_has-desirability_for_ (_in-role NIL Enable) 0 ?Everyone)
...
(user-query-for_bindings-of_in_
 1 ;<up to number>/All
 ?A1
 (_doing_is-best-action-for_ User ?A1 (file_removed-from_ ?file1 ?dir1)) )
...
(_is-able-to-do_ User (rm-i_from_ ?file ?dir)) ;a much too general assumption!!
...
(_has-desirability_for_ (_in-role_ (file_removed-from_ ?file ?dir) Result)
			-10 ;bad in general (but the goal in this case!!)
			?person )
...
(_has-desirability_for_ (_in-role_ (_types_ ?Person (avg-num-of-symbols ?num))
				   Enable )
			(- ?num) ;but should be made rel easy to other req's
			?Person )

;; 2nd (and last) option for removing a file:

(_is-able-to-do_ User (mv_from_to-dir_ ?file ?dir Slash-tmp))
(_has-influence_ Result
